...
derived requirements, and the CONOPS itself. You are FORBIDDEN from editing
`requirements/signoffs.yaml`. You are FORBIDDEN from running `make signoff`.
...
3. For each task in order: dispatch the matching Codex sub-agent from
   `.codex/agents/`, passing the feature slug. **Advance only when that task's
...
4. After each sub-agent returns, check `workflow/<feature>/questions.md`; if it
   has an unanswered `Q`, relay it using an available user-question tool or ask
   directly in the main conversation, write the `A:` back, and re-dispatch so
   the sub-agent resumes.
