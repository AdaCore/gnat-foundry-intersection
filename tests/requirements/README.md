...
| **Requirements** (this directory) | `tests/requirements/test_*.py` | The actual `bin/qemu_mps2/traffic_light` running under QEMU, talking the wire protocol over two TCP-routed UARTs |

## Running

```bash
# 1. Build the QEMU binary (its own Alire crate; emits bin/qemu_mps2/traffic_light).
