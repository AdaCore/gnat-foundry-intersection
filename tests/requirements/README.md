...
|-------|----------|--------|
| Unit | `tests/` (AUnit harness generated by gnattest) | In-process — sequencer + cmd parser + ped state |
| Proof | `gnatprove -P traffic_light.gpr` (default project, silver level) | SPARK proof of FR-SF-01 / FR-SF-02 on the conflict matrix |
| **Requirements** (this directory) | `tests/requirements/test_*.py` | The actual `bin/qemu_zynq7000/traffic_light` running under QEMU, talking the wire protocol over two TCP-routed UARTs |

## Running

```bash
# 1. Build the QEMU binary (its own Alire crate; emits bin/qemu_zynq7000/traffic_light).
...
2. Spawns `qemu-system-arm -M xilinx-zynq-a9 -m 1G -nographic`
...
Some requirements have no observable equivalent on the QEMU
xilinx-zynq-a9 target. They are left to other layers (hardware bring-up,
code review, or upstream SPARK proof).
...
| FR-PD-08 (visible request indicator) | The button-mounted LED is GPIO; no equivalent in this build (HAL `Set_Lamp` is a no-op on `qemu_zynq7000`). |
| FR-SF-06 (≥1 Hz MMU heartbeat) | A discrete GPIO pulse on a separate channel; no MMIO routing on xilinx-zynq-a9. Distinct from the FR-UI-04 UART0 heartbeat, which **is** tested above. |
...
| NFR-RL-01..03 | RAM/CRC/clock self-tests + IWDG/WWDG — all device-specific; no equivalent peripheral on xilinx-zynq-a9. |
| NFR-DG-02 | Fault-code retention in non-volatile memory — no flash on xilinx-zynq-a9 in this build. |
...
- FR-PD-* tests reaching NS/EW_THROUGH_GREEN with ped: ~33–55 s
