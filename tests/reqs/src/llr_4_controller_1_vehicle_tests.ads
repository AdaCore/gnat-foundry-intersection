...
package Llr_4_Controller_1_Vehicle_Tests is
   ...
   --  alphabet and over the timer, so its routine runs all twenty-two
   --  sequencer states rather than a sample of them. Statement 1 is verified
   --  by analysis and has no routine here (see its verified_by annotation).
   procedure Test_02_Vehicle_Held_While_Dwell_Remains (T : in out Test);

   --  Moore output rows (statements 3-24), in statement order.
   ...
   procedure Test_07_NS_Both_Through_Hold_Faces (T : in out Test);
   ...
   procedure Test_18_EW_Both_Through_Hold_Faces (T : in out Test);
   ...
   --  Transitions (statements 25-50), in statement order.
   --
