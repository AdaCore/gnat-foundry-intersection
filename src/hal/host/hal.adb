...
package body HAL is
   ...
   function To_U is new
     Ada.Unchecked_Conversion (Interfaces.C.int, Unsigned_32);
   function To_I is new
     Ada.Unchecked_Conversion (Unsigned_32, Interfaces.C.int);
   ...
   begin
      ...
      if Flags >= 0 then
         Rc :=
           C_Fcntl_Set (STDIN_FD, F_SETFL, To_I (To_U (Flags) or O_NONBLOCK));
      end if;
   ...
   procedure Set_Dont_Walk
     (CW : Crosswalk; Steady : Boolean; Flashing : Boolean) is
   begin
      Put_Line
        ("[HAL/host] dontwalk "
         & Crosswalk'Image (CW)
         & " steady="
         & Boolean'Image (Steady)
         & " flash="
         & Boolean'Image (Flashing));
   end Set_Dont_Walk;
