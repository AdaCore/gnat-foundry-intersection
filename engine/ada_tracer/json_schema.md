...
| `checks` | array | The `--@covers`-tagged pragmas and aspect associations, project-wide, sorted by (file, line); see [Check](#check) |
...
times and each occurrence has its own line. `tests/core/controller-test_data-tests.adb`
...
One `--@covers`-tagged pragma or aspect association. Only tagged constructs
are reported — the tag is an opt-in claim that the construct verifies a
requirement; an untagged pragma or contract is not a check. The tag must open
its own comment line on the line(s) directly above the construct; a first
aspect may instead be tagged above the `with` that opens its aspect list,
which is how a one-line `with Post => ...` is annotated.

| Field | Type | Meaning |
|---|---|---|
| `kind` | string | `"pragma"` or `"aspect"` |
| `name` | string | The pragma name or aspect mark as written, e.g. `"Compile_Time_Error"`, `"Post"`, `"No_Return"` |
| `location` | object | `{ "file": string, "line": integer, "column": integer }` — the construct, not the tag |
| `covers` | array | The raw `@covers` payloads, one string per tag line, in source order |
...
`tests/core/conflicts-test_data-tests.adb` — the second block, because the first
...
`packages` is sorted by name and `checks` by (file, line); `subprograms`,
`entities`, `library_subprograms` and `body_comments` keep source order. Two
runs over unchanged sources therefore produce byte-identical output and the
document diffs cleanly. Non-ASCII characters are emitted as `\uXXXX` escapes.
...
`make test-inventory` on every `make trace-check` / `make trace`, and a run
