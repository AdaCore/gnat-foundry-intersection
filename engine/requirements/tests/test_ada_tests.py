...
FILE = "src/tests/conflicts-test_data-tests.adb"
...
def test_none_tag_marks_derived(tmp_path: Path) -> None:
    ...
    file = "src/tests/common/display-test_data-tests.adb"
    node = load(
        tmp_path,
        package(
...
def test_untagged_routine_has_no_refs_and_is_not_derived(tmp_path: Path) -> None:
    ...
    file = "src/tests/states-test_data-tests.adb"
    node = load(
        tmp_path,
        package(
...
def test_library_subprograms_are_searched_too(tmp_path: Path) -> None:
    ...
    path = write_inventory(
        ...
        library_subprograms=[gnattest_routine("Test_Loose", "src/tests/loose-tests.adb", line=5)],
    )
    inventory = Inventory.model_validate_json(path.read_text(encoding="utf-8"))
    assert set(TestSet.from_inventory(inventory, path).nodes) == {"loose.Test_Loose"}
...
def test_empty_inventory_yields_empty_set(tmp_path: Path) -> None:
    """An inventory with no packages is an empty set, not an error."""
