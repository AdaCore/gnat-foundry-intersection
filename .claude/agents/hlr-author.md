---
name: hlr-author
description: Elaborate high-level requirements (HLR) for a feature. Use for the HLR task of the feature workflow — authors/edits requirements/hlr/*.yaml traced to the CONOPS. Oracle is `make validate-reqs TRACE_LAYERS=CONOPS,HLR`.
...
`make validate-reqs TRACE_LAYERS=CONOPS,HLR`; you are done **only** when it exits
