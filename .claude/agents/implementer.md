---
name: implementer
description: Implement the LLRs in Ada for a feature. Use for the Implementation task of the feature workflow — writes src/*.ads/.adb per the architecture and code conventions. Oracle is `make check && make build-native && make trace-check-code && make test`.
