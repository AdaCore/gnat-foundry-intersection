...
description: Extend tests until GNATcov reports full coverage. Use for the Coverage task of the feature workflow — adds/extends requirements-based tests under tests/reqs/ to close stmt+MCDC gaps. Oracle: `make all-coverage && make check-coverage`.
...
`make all-coverage && make check-coverage`; you are done **only** when the gate
exits zero (`no SID file found` warnings are benign).
